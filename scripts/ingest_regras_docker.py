"""
Script para indexar regras no Qdrant quando rodando via Docker Compose.

Este script usa as mesmas variáveis de ambiente que a API usa no Docker,
conectando no Qdrant do container (qdrant:6333) ao invés do localhost.

Uso:
    # Opção 1: Rodar dentro do container Docker
    docker-compose exec langgraph-api python scripts/ingest_regras_docker.py
    
    # Opção 2: Rodar localmente apontando para Qdrant do Docker
...
def main():
    ...
    print(f"\nRegras indexadas no Qdrant: {QDRANT_HOST}:{QDRANT_PORT}")
    print("\nProximos passos:")
    print("   1. Teste a busca via API: POST /agent/invoke")
    print("   2. Verifique retrieved_rules no estado do grafo")
